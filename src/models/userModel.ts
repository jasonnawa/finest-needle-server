...
export default class UserModel {
  ...
  async findAll() {
    return await UserMongooseModel.find({ paymentStatus: PaymentStatus.PAID })
      .select("-password -__v")
      .populate("preference")
      .lean();
  }
  ...
  async deleteManyUnpaidStatus() {
    ...
    return await UserMongooseModel.deleteMany({
  ...
  async updateUser(user: Partial<IUser>) {
    ...
    return await UserMongooseModel.findByIdAndUpdate(_id, updateData, {
      new: true,
    });
  }
